...
public interface CounsellorBookingRepository extends JpaRepository<CounsellorBooking, String> {
    ...
    @Query("SELECT b FROM CounsellorBooking b WHERE b.user.mrnId = :mrnId ORDER BY b.bookingTime DESC")
    Optional<CounsellorBooking> findFirstByMrnIdOrderByBookingTimeDesc(@Param("mrnId") String mrnId);

    // Check if a user has any bookings by their MRN ID
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN TRUE ELSE FALSE END FROM CounsellorBooking b WHERE b.user.mrnId = :mrnId")
